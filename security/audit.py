...
class AuditSequenceCounter:
    def __init__(self, sequence_file_path=None):
        ...
        self._current_id = 0
        self.sequence_file = sequence_file_path if sequence_file_path else os.path.join(get_base_dir(), "logs", "audit_sequence.txt")
        self._load()
...
class SecurityEvent:
    def __init__(self, event_type: str, actor: str, action: str, status: str, details: str = "", context: dict = None):
...
class PermissionDenied(SecurityEvent):
    def __init__(self, actor: str, action: str, details: str = "", context: dict = None):
        super().__init__("PermissionDenied", actor, action, "DENIED", details, context)

class HashMismatch(SecurityEvent):
    def __init__(self, filepath: str, action: str, details: str = "", context: dict = None):
        super().__init__("HashMismatch", "System", action, "FAILED", f"File: {filepath} | {details}", context)
...
class UnauthorizedCommand(SecurityEvent):
    def __init__(self, actor: str, command: str, details: str = "", context: dict = None):
        super().__init__("UnauthorizedCommand", actor, f"Execute: {command}", "DENIED", details, context)

class ConfigTampered(SecurityEvent):
    def __init__(self, filepath: str, details: str = "", context: dict = None):
        super().__init__("ConfigTampered", "System", "ValidateConfig", "FAILED", f"Config: {filepath} | {details}", context)
