...
try:
    ...
    ctypes.windll.shell32.SetCurrentProcessExplicitAppUserModelID(
        "vigilo.security.uninstaller.v3"
    )
except Exception:
    pass
...
class UninstallerApp(tk.Tk):
    def __init__(self):
        ...
        tk.Label(
            top,
            text="🐕  VigiLo Security Uninstaller",
...
class ConfirmPage(ttk.Frame):
    def __init__(self, parent, controller):
        ...
        # Header
        ttk.Label(self, text="Uninstall VigiLo", style="Header.TLabel").pack(
            pady=(6, 2), anchor="w"
        )
        ttk.Label(
            self,
            text="This will completely remove VigiLo Security from your system.",
        ...
        for icon, label, detail in items:
            ...
            tk.Label(row, text=icon, font=("Segoe UI Emoji", 11), bg=BG2, fg=FG).pack(
                side="left", padx=(0, 8)
            )
            tk.Label(
                row, text=label, font=("Segoe UI", 10, "bold"), bg=BG2, fg=FG
            ).pack(side="left")
            tk.Label(
                row, text=f"  —  {detail}", font=("Segoe UI", 9), bg=BG2, fg=DIM
            ).pack(side="left")
        ...
        if not is_admin():
            ...
            tk.Label(
                adm_row,
                text="⚠️  Administrator privileges are required to perform uninstallation.",
...
class UninstallPage(ttk.Frame):
    def __init__(self, parent, controller):
        ...
        ttk.Label(self, text="Removing VigiLo…", style="Header.TLabel").pack(
            pady=(6, 4), anchor="w"
        )
        self.status_var = tk.StringVar(value="Initializing uninstallation...")
        ttk.Label(self, textvariable=self.status_var, style="SubHeader.TLabel").pack(
            anchor="w", pady=(0, 8)
        )
    ...
    def _run(self):
        ...
        if success:
            ...
            messagebox.showwarning(
                "Uninstallation Complete with warnings",
...
class DonePage(ttk.Frame):
    def __init__(self, parent, controller):
        ...
        ttk.Label(self, text="Uninstall Complete", style="Header.TLabel").pack(
            pady=(8, 4), anchor="w"
        )
        ttk.Label(
            self,
            text="VigiLo Security has been fully removed from this system.",
            style="SubHeader.TLabel",
        ...
        removed = [
            (
                "✓",
                "Scheduled tasks removed successfully",
                "AntiTheft_Service & AntiTheft_Commander",
            ),
            ("✓", "Registry startup hooks cleaned", "HKLM Run key removed"),
            ("✓", "Installation binaries deleted", INSTALL_DIR),
            (
                "✓",
                "ProgramData captures buffer cleaned",
                "Offline image caches removed",
            ),
        ]
        for icon, title, detail in removed:
            ...
            tk.Label(
                row, text=icon, font=("Segoe UI", 11, "bold"), fg=GREEN, bg=BG2
            ).pack(side="left", padx=(0, 10))
            tk.Label(
                row, text=title, font=("Segoe UI", 10, "bold"), fg=FG, bg=BG2
            ).pack(side="left")
            tk.Label(
                row, text=f"  —  {detail}", font=("Segoe UI", 9), fg=DIM, bg=BG2
            ).pack(side="left")
        ...
        tk.Label(
            btn_bar,
            text="© 2026 VigiLo Security  —  All Rights Reserved",
            font=("Segoe UI", 8),
            fg=DIM,
            bg=BG,
        ).pack(side="left")
...
if __name__ == "__main__":
    ...
    if is_admin():
        ...
        if (
            INSTALL_DIR.lower() in current_dir
            or INSTALL_DIR.lower() in sys.executable.lower()
        ):
            try:
