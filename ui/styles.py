...
def apply_styles(style):
    ...
    style.configure(
        "Header.TLabel", font=("Segoe UI", 20, "bold"), foreground=ACCENT, background=BG
    )
    style.configure(
        "SubHeader.TLabel", font=("Segoe UI", 10), foreground=DIM, background=BG
    )
    style.configure(
        "Warning.TLabel", font=("Segoe UI", 10), foreground=WARN, background=BG
    )
    ...
    style.configure(
        "Horizontal.TProgressbar", troughcolor="#3c3c3c", background=ACCENT, thickness=8
    )
