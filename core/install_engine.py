...
class InstallEngine:
    ...
    def deploy(self, src_exe, src_uninstall, telegram_config, progress_callback=None, log_callback=None):
        ...
        try:
            ...
            try:
                cmd_acl = [
                    'icacls', self.install_dir, 
                    '/inheritance:r', 
                    '/grant:r', 'NT AUTHORITY\\SYSTEM:(OI)(CI)(F)', 
                    '/grant:r', 'BUILTIN\\Administrators:(OI)(CI)(F)', 
                    '/grant:r', 'BUILTIN\\Users:(OI)(CI)(RX)'
                ]
                subprocess.run(cmd_acl, check=True, capture_output=True, creationflags=subprocess.CREATE_NO_WINDOW)
            ...
            if not os.path.exists(src_exe):
                raise FileNotFoundError(f"Source payload executable not found: {src_exe}")
            ...
            if src_uninstall and os.path.exists(src_uninstall):
                ...
                report("Warning: uninstaller source payload not found. Skipping uninstaller deployment.", 50)
            ...
            from config.schema import TelegramConfig, AppConfig, SecurityConfig, CameraConfig
            mgr = ConfigManager(self.dest_config)
            app_cfg = AppConfig(
                telegram=TelegramConfig(telegram_config.get("bot_token"), telegram_config.get("chat_id")),
            ...
            subprocess.run(["schtasks", "/Run", "/TN", "VigiLo_Service"], capture_output=True, creationflags=subprocess.CREATE_NO_WINDOW)
            subprocess.run(["schtasks", "/Run", "/TN", "VigiLo_Commander"], capture_output=True, creationflags=subprocess.CREATE_NO_WINDOW)
