...
class TelegramClient:
    ...
    def send_message(self, text, parse_mode="Markdown"):
        ...
        if not self.bot_token or not self.chat_id or "YOUR_BOT_TOKEN" in self.bot_token:
            logger.warning("Telegram credentials not configured. Skipping message upload.")
            return False
        ...
        try:
            ...
            if resp.status_code == 200:
                return True
            logger.error(f"Telegram send_message failed: HTTP {resp.status_code} - {resp.text}")
    ...
    def send_photo(self, image_path, caption=None):
        ...
        if not self.bot_token or not self.chat_id or "YOUR_BOT_TOKEN" in self.bot_token:
            logger.warning("Telegram credentials not configured. Skipping photo upload.")
            return False
        ...
        try:
            with open(image_path, "rb") as img:
                ...
                if resp.status_code == 200:
                    return True
                logger.error(f"Telegram send_photo failed: HTTP {resp.status_code} - {resp.text}")
        except Exception as e:
    ...
    def send_audio(self, audio_path, caption=None):
        ...
        if not self.bot_token or not self.chat_id or "YOUR_BOT_TOKEN" in self.bot_token:
            logger.warning("Telegram credentials not configured. Skipping audio upload.")
            return False
        ...
        try:
            with open(audio_path, "rb") as audio:
                ...
                if resp.status_code == 200:
                    return True
                logger.error(f"Telegram send_audio failed: HTTP {resp.status_code} - {resp.text}")
        except Exception as e:
    ...
    def send_document(self, doc_path, caption=None):
        ...
        if not self.bot_token or not self.chat_id or "YOUR_BOT_TOKEN" in self.bot_token:
            logger.warning("Telegram credentials not configured. Skipping document upload.")
            return False
        ...
        try:
            with open(doc_path, "rb") as doc:
                ...
                if resp.status_code == 200:
                    return True
                logger.error(f"Telegram send_document failed: HTTP {resp.status_code} - {resp.text}")
        except Exception as e:
