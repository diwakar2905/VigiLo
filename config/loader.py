...
class ConfigLoader:
    ...
    def load(config_path):
        ...
        if not os.path.exists(config_path):
            ...
            logger.warning(
                f"ConfigLoader: Active configuration missing at {config_path}. Checking backups..."
            )
            if backup_mgr.restore_latest_valid_backup():
                logger.info(
                    "ConfigLoader: Restored configuration from backup successfully."
                )
            else:
                logger.info(
                    "ConfigLoader: No backup found. Instantiating default configuration."
                )
                return AppConfig()
        ...
        try:
            ...
            logger.error(
                f"ConfigLoader: Active configuration file is corrupted (invalid JSON): {je}"
            )
            is_corrupted = True
            ...
            if os.path.exists(meta_path):
                try:
                    ...
                    if actual_hash != expected_hash:
                        logger.error(
                            f"ConfigLoader: Integrity violation detected! Expected: {expected_hash}, Actual: {actual_hash}"
                        )
                    ...
                    logger.warning(
                        f"ConfigLoader: Failed to verify config metadata: {me}"
                    )

        # 2. Trigger Restore on Corruption
        if is_corrupted:
            logger.warning(
                "ConfigLoader: Attempting to restore configuration from backups..."
            )
            if backup_mgr.restore_latest_valid_backup():
                ...
                try:
                    ...
                    raise LoaderError(
                        f"Failed to load configuration after restoration: {e}"
                    )
            else:
                logger.critical(
                    "ConfigLoader: Active config is corrupted and no backups could be restored. Using defaults."
                )
                return AppConfig()
        ...
        try:
            ...
            fv_data = data.get("face_verification", {})
            ...
            raw_embeddings = fv_data.get("reference_embeddings", [])
            decrypted_embeddings = []
            for raw_emb in raw_embeddings:
                decrypted_embeddings.append(decrypt_data(raw_emb))

            # Decrypt vault key
            raw_vault_key = v_data.get("vault_key", "")
            vault_key = decrypt_data(raw_vault_key)

            # Decrypt whatsapp credentials
            wa_data = data.get("whatsapp", {})
            raw_wa_access_token = wa_data.get("access_token", "")
            raw_wa_recipient = wa_data.get("recipient_phone", "")

            wa_access_token = decrypt_data(raw_wa_access_token)
            wa_recipient = decrypt_data(raw_wa_recipient)
            ...
            face_verification = FaceVerificationConfig(
                ...
                threshold=fv_data.get("threshold", 0.363),
                reference_embeddings=decrypted_embeddings,
            )
            vault = VaultConfig(
                enabled=v_data.get("enabled", True),
                target_dir=v_data.get("target_dir", "C:\\VigiLoVault"),
                vault_key=vault_key,
            )
            whatsapp = WhatsAppConfig(
                enabled=wa_data.get("enabled", False),
                phone_number_id=wa_data.get("phone_number_id", ""),
                access_token=wa_access_token,
                recipient_phone=wa_recipient,
            )
