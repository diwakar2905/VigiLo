...
class ConfigSaver:
    ...
    def save(config_path, app_config):
        ...
        try:
            ...
            for emb in app_config.face_verification.reference_embeddings:
                encrypted_embeddings.append(encrypt_data(emb))
            data["face_verification"]["reference_embeddings"] = encrypted_embeddings

            # Encrypt vault key via DPAPI
            data["vault"]["vault_key"] = encrypt_data(app_config.vault.vault_key)

            # Encrypt whatsapp credentials via DPAPI
            data["whatsapp"]["access_token"] = encrypt_data(
                app_config.whatsapp.access_token
            )
            data["whatsapp"]["recipient_phone"] = encrypt_data(
                app_config.whatsapp.recipient_phone
            )
